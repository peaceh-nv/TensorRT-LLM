...
class Scenario:
    ...
    def __post_init__(self) -> None:
        ...
        assert (self.kv_len != 0 or self.qo_len
                <= self.kv_cache_len), "Seq len larger than cache len"
...
def kv_cache_manager_from(Attention: type[AttentionBackend],
                          s: Scenario) -> KVCacheManager:
    ...
    cache_type = (tensorrt_llm.bindings.internal.batch_manager.CacheType.CROSS
                  if s.cross else
...
def produce_outputs(
    ...
    *,
    ...
    num_cached_tokens_per_seq = [(num_cached_tokens if isinstance(
        num_cached_tokens, int) else num_cached_tokens(i))
                                 for i in range(s.batch_size)]

    kv_cache_params = KVCacheParams(
        use_cache=True, num_cached_tokens_per_seq=num_cached_tokens_per_seq)
    kv_cache_manager = kv_cache_manager_from(Attention, s)
...
def generate_causal_mask(seq_lens, qo_lens, batch_size, dtype):
    ...
    for i in range(batch_size):
        ...
        causal_mask = torch.nn.functional.pad(
            causal_mask,
            (0, max_seq_len - kv_len, 0, max_qo_len - qo_len),
            "constant",
...
def test_attention_backend_ifb(s: PagedScenario):
    ...
    def produce(Attention: type[AttentionBackend], kv_cache: torch.Tensor):
        return produce_outputs(
            ...
            s,
            kv_cache=kv_cache,
            num_cached_tokens=lambda i:
            (num_cached_tokens_prefill
    ...
    causal_mask = (generate_causal_mask(kv_lens, qo_lens, batch_size, dtype)
                   if s.causal else 0)

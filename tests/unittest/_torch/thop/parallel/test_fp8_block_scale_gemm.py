...
def test_cute_dsl_mxfp8_gemm_rubin_k128_replicated_scales():
    ...
    a_fp8, a_sf = \
        torch.ops.trtllm.fp8_quantize_1x128_packed_ue8m0(a)
...
@pytest.mark.skipif(
    ...
    reason="The test requires public Rubin CuTe DSL support.",
)
...
def test_cute_dsl_mxfp8_gemm_rubin_mixed_clusters_clc_dynamic_prefetch_multi_wave(
    ...
    """Mixed-CGA MXFP8 supports CLC scheduling with prefetch enabled."""
    from tensorrt_llm._torch.custom_ops.cute_dsl_custom_ops import \
        CuteDSLMXFP8RubinLinear
    ...
    assert runner.should_profile_tactic_in_subprocess(

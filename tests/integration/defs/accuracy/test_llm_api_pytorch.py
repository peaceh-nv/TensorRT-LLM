...
from ..conftest import (check_device_contain, get_device_count,
                        ...
                        skip_pre_hopper, skip_pre_rubin, skip_ray, skip_x86)
...
class TestLlama3_1_8B(LlmapiAccuracyTestHarness):
    ...
    @skip_pre_blackwell
    def test_nvfp4(self, nvfp4_gemm_backend):
        model_path = f"{llm_models_root()}/nvfp4-quantized/Meta-Llama-3.1-8B"
        sm_version = get_sm_version()
        if nvfp4_gemm_backend == "cutedsl" and sm_version not in (100, 103, 107):
            pytest.skip(f"cutedsl nvfp4_gemm_backend supports SM 100, 103, 107 only")

        llm_kwargs = {}
        if nvfp4_gemm_backend is not None:
            llm_kwargs["nvfp4_gemm_config"] = Nvfp4GemmConfig(
    ...
    @skip_pre_rubin
    def test_cute_dsl_bf16_gemm_tunable_rubin_kernels(self):
        with LLM(
            self.MODEL_PATH,
    ...
    @skip_pre_rubin
    def test_nvfp4_cutedsl_mixed_cluster_tunable_rubin_kernels(self):
        model_path = f"{llm_models_root()}/nvfp4-quantized/Meta-Llama-3.1-8B"
        with LLM(
            model_path,
            enable_autotuner=True,
            nvfp4_gemm_config=Nvfp4GemmConfig(allowed_backends=["cutedsl"]),
        ) as llm:
            assert llm.args.quant_config.quant_algo == QuantAlgo.NVFP4
            task = CnnDailymail(self.MODEL_NAME)
            task.evaluate(llm)
...
class TestDeepSeekV3Lite(LlmapiAccuracyTestHarness):
    ...
    def test_nvfp4(self, fp8kv, attention_dp, cuda_graph, overlap_scheduler,
        ...
        if moe_backend == "CUTEDSL" and sm_version not in (100, 103, 107):
            pytest.skip(
                f"{moe_backend} backend supports SM 100, 103, and 107 only")
        if nvfp4_gemm_backend == "cutedsl" and sm_version not in (100, 103,
                                                                  107):
            pytest.skip(
                "cutedsl nvfp4_gemm_backend supports SM 100, 103, 107 only")
    ...
    @parametrize_with_ids("torch_compile", [False, True])
    @parametrize_with_ids(
        "fp8kv,attention_dp,cuda_graph,overlap_scheduler,low_precision_combine",
        [
            ...
            # low_precision_combine only valid with attention_dp=True
...
class TestDeepSeekR1(LlmapiAccuracyTestHarness):
    ...
    @skip_pre_blackwell
    @pytest.mark.parametrize(
        "tp_size,pp_size,ep_size,mtp_nextn,fp8kv,attention_dp,enable_lm_head_tp_in_adp,cuda_graph,overlap_scheduler,max_batch_size,enable_locality_domains,moe_backend",
        [
        ...
        ids=[
            ...
            "latency_trtllmgen_adp_lmtp", "latency_adp_lmtp_tp4",
            "latency_adp_lmtp_tp4_locality_domain", "throughput",
    ...
    def test_nvfp4_multi_gpus(self, tp_size, pp_size, ep_size, mtp_nextn, fp8kv,
                              ...
                              enable_locality_domains, moe_backend):
        sm_version = get_sm_version()
        if moe_backend == "TRTLLM" and sm_version in (120, 121):
            pytest.skip(f"{moe_backend} backend does not support SM 120 or 121")
        ...
        kv_cache_config_kwargs = dict(free_gpu_memory_fraction=0.70)
        if enable_locality_domains:
            kv_cache_config_kwargs.update(free_gpu_memory_fraction=0.80,
...
class TestQwen3_8B(LlmapiAccuracyTestHarness):
    ...
    def test_bf16(self, tp_size, pp_size, ep_size, attention_dp, cuda_graph,
        ...
        pytorch_config = dict(
            disable_overlap_scheduler=not overlap_scheduler,
            cuda_graph_config=CudaGraphConfig(
                enable_padding=True, max_batch_size=32) if cuda_graph else None)
    ...
    def test_eagle3(self, eagle3_one_model, enable_chunked_prefill,
        ...
        cuda_graph_config = (CudaGraphConfig(
            enable_padding=True, max_batch_size=32) if enable_draft_len_schedule
    ...
    def test_dflash(self):
        ...
        dflash_model_dir = f"{llm_models_root()}/Qwen3-8B-DFlash-b16"
        target_model_dir = f"{llm_models_root()}/Qwen3/Qwen3-8B"
...
class TestQwen3_30B_A3B(LlmapiAccuracyTestHarness):
    ...
    @skip_post_blackwell
    @parametrize_with_ids("torch_compile", [False, True])
    @pytest.mark.parametrize("tp_size,ep_size,attention_dp,cuda_graph",
                             [(1, 1, False, False)],
                             ids=["latency"])
    def test_fp8_block_scales(self, tp_size, ep_size, attention_dp, cuda_graph,
                              torch_compile):
        torch_compile_config = _get_default_torch_compile_config(torch_compile)
    ...
    @parametrize_with_ids("torch_compile", [False, True])
    @pytest.mark.parametrize(
        "tp_size,pp_size,ep_size,attention_dp,moe_backend",
        [
            (1, 1, 1, False, "CUTLASS"),
            (1, 1, 1, False, "TRTLLM"),
            (1, 1, 1, False, "CUTEDSL"),
        ...
        ids=[
            ...
            "dep4_latency_moe_cutedsl",
            "tep4_latency_moe_cutlass",
            "tep4_latency_moe_trtllm",
            "tep4_latency_moe_cutedsl",
        ],
    )
    def test_nvfp4(
        self,
...
class TestQwen3_235B_A22B(LlmapiAccuracyTestHarness):
    ...
    @pytest.mark.parametrize("tp_size,pp_size,ep_size,attention_dp,moe_backend",
                             ...
                             ids=[
                                 "latency_deepgemm",
                                 "throughput_latency_deepgemm",
                                 "latency_trtllm", "throughput_latency_trtllm"
                             ])
    def test_fp8_block_scales(self, tp_size, pp_size, ep_size, attention_dp,
                              moe_backend):
        pytorch_config = dict(cuda_graph_config=CudaGraphConfig(),
    ...
    @pytest.mark.skip_less_mpi_world_size(8)
    @pytest.mark.parametrize(
        "tp_size,pp_size,ep_size,attention_dp,moe_backend,eagle3",
        [
            (8, 1, 8, True, "CUTLASS", False),
            (8, 1, 8, False, "TRTLLM", False),
            (8, 1, 8, True, "TRTLLM", False),
            (8, 1, 8, False, "TRTLLM", True),
            (8, 1, 8, True, "TRTLLM", True),
        ],
        ids=[
            "latency_moe_cutlass",
            "latency_moe_trtllm",
    ...
    def test_nvfp4(self, tp_size, pp_size, ep_size, attention_dp, moe_backend,
                   eagle3):
...
class TestGPTOSS(LlmapiAccuracyTestHarness):
    ...
    def test_dflash(self, mocker):
        ...
        dflash_model_dir = f"{llm_models_root()}/gpt-oss-20b-DFlash"
        target_model_dir = f"{llm_models_root()}/gpt_oss/gpt-oss-20b"
...
class TestQwen3_5_397B_A17B(LlmapiAccuracyTestHarness):
    ...
    @pytest.mark.skip_less_device(4)
    @pytest.mark.parametrize(
        "tp_size,ep_size,attention_dp,moe_backend,enable_block_reuse",
        [
            (4, 4, False, "CUTEDSL", False),
            (4, 4, True, "CUTEDSL", False),
            (4, 4, False, "TRTLLM", False),
            (4, 4, True, "TRTLLM", False),
            (4, 4, False, "CUTEDSL", True),
        ],
        ids=[
            "tep4_cutedsl",
            "adp4_cutedsl",
    ...
    def test_nvfp4(self, tp_size, ep_size, attention_dp, moe_backend,
                   enable_block_reuse, mocker):
    ...
    @pytest.mark.skip_less_device(4)
    def test_nvfp4_mtp3_gdn_replay_tep4(self, mocker):
        model_path = f"{llm_models_root()}/Qwen3.5-397B-A17B-NVFP4"
        if not os.path.exists(model_path):
            pytest.skip(f"Model directory {model_path} does not exist")

        replay_env = {"TRTLLM_USE_GDN_REPLAY": "1"}
        mocker.patch.dict(os.environ, replay_env)

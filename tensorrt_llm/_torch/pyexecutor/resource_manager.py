...
class KVCacheManager(BaseResourceManager):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        print(f"is_vswa: {self.is_vswa}")
        print(f"max_attention_window_vec: {self.max_attention_window_vec}")
        ...
        print(f"blocks_per_window: {blocks_per_window}")
        print(f"self.max_attention_window_vec: {self.max_attention_window_vec}")
        ...
        print(f"kv_connector_manager: {self.kv_connector_manager}")
    ...
    def copy_batch_block_offsets(self, dst_tensor: torch.Tensor,
                                 ...
                                 offset: int):
        self.impl.copy_batch_block_offsets(dst_tensor, request_ids, beam_width,
                                           offset)
...
class KVCacheManagerV2(BaseResourceManager):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        free_mem, total_mem = torch.cuda.mem_get_info()
        max_tokens = free_mem_fraction * free_mem / self.get_cache_bytes_per_token(
        )
        ...
        print(
            f"free_mem_fraction * free_mem: {free_mem_fraction * free_mem}, haha: {8000<< 20}"
        )

        if kv_cache_config.max_tokens is not None:
            if kv_cache_config.free_gpu_memory_fraction is not None:
                max_tokens = min(int(kv_cache_config.max_tokens * 3),
                                 max_tokens)
                logger.warning(
                    f'Both free_gpu_memory_fraction and max_tokens are set (to {free_mem_fraction} and {max_tokens} with free memory {free_mem / (1 << 32)} of total memory {total_mem / (1<<32)}, respectively). The smaller value will be used.'
                )
            else:
                max_tokens = int(kv_cache_config.max_tokens * 3)
                logger.info(
                    f"max_tokens is set by kv_cache_config.max_tokens: {max_tokens}"
                )
        ...
        print(f"self.quota: {self.quota.quota}")
        ...
        self.pool_to_layer_mapping_dict: dict[int, list[int]] = {}
        ...
        print(f"kv_cache_pool_mapping_list: {kv_cache_pool_mapping_list}")
        ...
        print(f'num_local_layers: {self.num_local_layers}')
        print(self.kv_connector_manager)
    ...
    def get_buffers(self, layer_idx: int) -> Optional[torch.Tensor]:
        ...
        return convert_to_torch_tensor(
            TensorWrapper(
                addr_key + page_size_key * layer_offset * self.kv_factor,
                self.dtype,
                (
    ...
    def get_num_available_tokens(self,
        ...
        return min(max_seq_len - 1, 10000000)
        return self.impl.clamp_max_seq_len_for_mem(batch_size, max_seq_len)

    def get_num_free_blocks(self) -> int:
        max_num_pages = self.impl.get_page_index_upper_bound(0, Role.KEY)
        return max_num_pages // self.num_local_layers
                ...
                if 'cp_type' in self.mapping.cp_config and CpType.STAR == self.mapping.cp_config[
                    ...
                    if req.is_first_context_chunk and self._kv_connector_should_add_sequence(
                        ...
                        if not self.enable_block_reuse:
                            ...
                            print(
                                f"enable_block_reuse: {req.py_request_id}, committed tokens: {kv_cache.num_committed_tokens}, context current position: {req.context_current_position}, context chunk size: {req.context_chunk_size}, prompt len: {req.prompt_len}"
                            )
    ...
    def get_kv_cache_stats(self):
        ...
        return A(allocated_bytes=self.quota.quota)
    ...
    def add_dummy_requests(
        ...
        for i, req_id in enumerate(request_ids):
            ...
            req.paged_kv_block_ids = []
            if prepare_resource:
                kv_cache = self.impl.create_kv_cache(req.lora_task_id,
                                                     input_tokens)
                assert kv_cache.num_committed_tokens == 0
                success = kv_cache.resume(
                    torch.cuda.current_stream().cuda_stream)
    ...
    def get_batch_cache_indices(
        ...
        return self.get_batch_cache_indices_by_pool_id(
            request_ids,
            pool_id=self.layer_to_pool_mapping_dict[layer_id],
            is_kv_aggregate=is_kv_aggregate)

    def get_batch_cache_indices_by_pool_id(
            self,
            request_ids: List[int],
            *,
            pool_id: int = 0,
        ...
        return list([
            i // div_factor
            for i in self.kv_cache_map[req_id].get_page_indices(pool_id)
    ...
    def update_resources(self,
        ...
        for req in scheduled_batch.context_requests:
            ...
            if self.enable_block_reuse:
                kv_cache = self.kv_cache_map[req.py_request_id]
                if req.context_current_position - 1 > kv_cache.num_committed_tokens:
                    kv_cache.commit(
                        req.get_tokens(0)[kv_cache.num_committed_tokens:req.
                                          context_current_position - 1])
                kv_cache.stop_committing()
            else:
                kv_cache = self.kv_cache_map[req.py_request_id]
    ...
    def copy_batch_block_offsets(self, dst_tensor: torch.Tensor,
        ...
        print(f"self.kv_cache_pool_pointers: {self.kv_cache_pool_pointers}")
        print(f"self.kv_cache_pool_mapping: {self.kv_cache_pool_mapping}")

        for pool_idx in range(self.num_pools):
            batch_indices_key = self.get_batch_cache_indices_by_pool_id(
                request_ids, pool_id=pool_idx, is_kv_aggregate=False)
            for idx, batch_idx_key in enumerate(batch_indices_key):
                dst_tensor[pool_idx, idx + offset,
                           0, :len(batch_idx_key)] = torch.tensor(
                               batch_idx_key,
                               dtype=torch.int32,
                               device="cpu",
                               pin_memory=True)
                dst_tensor[pool_idx, idx + offset,
                           1, :len(batch_idx_key)] = torch.tensor(
                               batch_idx_key,
                               dtype=torch.int32,
                               device="cpu",
                               pin_memory=True) + 1

...
if IS_CUTLASS_DSL_AVAILABLE:
    ...
    if IS_CUTLASS_DSL_RUBIN_AVAILABLE:
        # SM107 dense GEMM / BMM custom ops
        # ====================================================================
        # Op-level wrappers for the SM107 dense kernels: FP8 blockwise GEMM /
            ...
            def get_valid_tactics(
                ...
                **kwargs,
                ...
                if (sm_version := get_sm_version()) != 107:
                    logger.debug(
                        f"CuteDSL: SM version {sm_version} is not supported. "
                        "CuteDSL FP8 blockwise GEMM only supports SM107.")
                    return []
                ...
                return [(use_2cta_instrs, mma_tiler, mma_inst_shape,
                        ...
                        mma_inst_shape in _SM107_FP8_BLOCKWISE_MMA_CANDIDATES
            ...
            def get_valid_tactics(
                ...
                **kwargs,
                ...
                if get_sm_version() != 107:
                    logger.debug("CuteDSL FP8 SM107 BMM only supports SM107. "
                                 "Skipping all tactics.")
                    return []
        ...
        class CuteDSLFp8RubinBmmQuantRunner(CuteDSLFp8RubinBmmRunner):
            """SM107 FP8 BMM runner with dynamic packed output scales."""
        ...
        def cute_dsl_fp8_bmm_rubin(
            ...
            if output_dtype != torch.bfloat16:
                raise ValueError(
                    "CuteDSL FP8 SM107 BMM only supports bfloat16 output, "
                    f"got {output_dtype}")
            if get_sm_version() != 107:
                raise ValueError(
                    f"CuteDSL: SM version {get_sm_version()} is not supported. "
                    "CuteDSL FP8 SM107 BMM only supports SM107.")
        ...
        def _(
            ...
            assert output.dtype == torch.bfloat16, \
                "CuTe DSL FP8 SM107 BMM output dtype must be BF16"
            assert output.shape == (batch_size, m, n), \
                "CuTe DSL FP8 SM107 BMM output shape is incorrect"
        ...
        class CuteDSLBlockScaledRubinLinear(TunableRunner):
            ...
            def get_valid_tactics(
                ...
                **kwargs,
                ...
                if (sm_version := get_sm_version()) != 107:
                    logger.debug(
                        f"CuteDSL: SM version {sm_version} is not supported. "
                        f"CuteDSL {self.gemm_name} SM107 runner. "
                ...
                # SM107 dense block-scaled tile sizes.
            ...
            def forward(
                ...
                **kwargs,
                ...
                """
                Performs a dense block-scaled GEMM using CuTe DSL on SM107.
                ...
                if not isinstance(use_prefetch, bool):
                    raise ValueError(
                        "CuteDSL SM107 use_prefetch must be Boolean")
                if (not isinstance(split_k, int) or isinstance(split_k, bool)
                        or split_k < 1):
                    raise ValueError(
                        f"CuteDSL SM107 split_k must be a positive integer, got {split_k!r}"
                    )
                ...
                # Blackwell NVFP4 and SM107 BF16 runners. Keying on
                ...
                if cache_key not in self.__class__.kernel_cache:
                    ...
                    # The SM107 kernels take mma_inst_shape and mma_tiler as 3-tuples.
                    if kernel_variant == "mixed_clusters":
        ...
        class CuteDSLNVFP4RubinLinear(CuteDSLBlockScaledRubinLinear):
            """SM107 NVFP4 runner backed by the persistent dense kernel."""
        ...
        def cute_dsl_nvfp4_gemm_inplace_rubin(
            ...
            """SM107 NVFP4 GEMM variant that writes into output_tensor."""
            if (sm_version := get_sm_version()) != 107:
                raise ValueError(
                    f"CuteDSL NVFP4 SM107 backend requires SM107, but got SM{sm_version}."
                )
            if partition_id < 0 or partition_id >= 2:
                raise ValueError(
                    "partition_id must be 0 or 1 when output_tensor is provided."
        ...
        def cute_dsl_nvfp4_gemm_locality_domain_inplace_rubin(
            ...
            """Tune and launch both locality domain NVFP4 GEMM partitions on SM107."""
            if (sm_version := get_sm_version()) != 107:
                raise ValueError(
                    f"CuteDSL NVFP4 SM107 backend requires SM107, but got SM{sm_version}."
                )
        ...
        def cute_dsl_fp8_per_tensor_gemm_rubin(
            ...
            if (sm_version := get_sm_version()) != 107:
                raise ValueError(
                    f"CuteDSL: SM version {sm_version} is not supported. "
                    f"CuteDSL fp8 per-tensor gemm only supports SM107.")
        ...
        # The fused FC12 kernel module ships with the CuteDSL MoE backend
        # change. Keep the op registration optional so that importing this
        # module still succeeds on an internal CuTe DSL build that does not
        # yet carry the kernel; the MoE backend checks for the op before use.
        try:
            ...
            Sm107BlockScaledContiguousGroupedGemmFusedFc12Kernel = None
            logger.debug(
                "Fused FC12 CuteDSL kernel is unavailable; "
                "trtllm::cute_dsl_nvfp4_fc12_fused_rubin is not registered.")

        if Sm107BlockScaledContiguousGroupedGemmFusedFc12Kernel is not None:
            ...
            class Fc12FusedInputsHelper(GatherGroupedGemmInputsHelper):
                ...
                def inputs_pre_hook(
                    ...
                    # (feed two None placeholders so it skips the
                    # locality-domain output resize).
                    base = list(super().inputs_pre_hook(
                        list(inputs[:10]) + [None, None]))
                    fc1_prefix = base[:10]
            ...
            class Sm107BlockScaledContiguousGroupedGemmFusedFc12Runner(
                ...
                def get_tuning_config(self) -> TuningConfig:
                    key = self.unique_id()
                    if key not in self.__class__.tuning_config_cache:
                        helper = Fc12FusedInputsHelper(self.num_experts,
                                                       self.top_k,
                                                       self.num_local_experts,
                                                       self.local_expert_offset,
                                                       self.tile_size)
                        self.__class__.tuning_config_cache[key] = TuningConfig(
                            ...
                            constraint_specs=(
                                ...
                                ConstraintSpec(2, 0,
                                               helper.infer_shape_num_tokens),
                                ConstraintSpec(
                                    5, 0, helper.infer_shape_max_num_tiles),
                                ConstraintSpec(
                                    6, 0, helper.infer_shape_max_num_tiles),
                                # fc2_c/fc2_routing_scales dim0 = num_tokens (follows M bucket like inputs 0/2)
                                ConstraintSpec(13, 0,
                                               helper.infer_shape_num_tokens),
                                ConstraintSpec(14, 0,
                                               helper.infer_shape_num_tokens),
                                # expanded_idx_to_permuted_idx dim0 = num_tokens too
                                ConstraintSpec(15, 0,
                                               helper.infer_shape_num_tokens),
                            ),
                            inputs_pre_hook=helper.inputs_pre_hook,
                ...
                def forward(self, inputs: List[torch.Tensor],
                    ...
                    fc1_norm_const_ptr = make_ptr(cutlass.Float32,
                                                  ...
                                                  cute.AddressSpace.gmem)
                    fc1_alpha_ptr = make_ptr(cutlass.Float32,
                                             fc1_alpha.data_ptr(),
                                             cute.AddressSpace.gmem)
                    fc2_alpha_ptr = make_ptr(cutlass.Float32,
                                             fc2_alpha.data_ptr(),
                                             cute.AddressSpace.gmem)
                    tile_idx_to_group_idx_ptr = make_ptr(
                        cutlass.Int32, tile_idx_to_group_idx.data_ptr(),
                    ...
                    num_non_exiting_tiles_ptr = make_ptr(
                        cutlass.Int32, num_non_exiting_tiles.data_ptr(),
                        cute.AddressSpace.gmem)
                    fc1_ready_ptr = make_ptr(cutlass.Int32,
                                             fc1_ready.data_ptr(),
                                             cute.AddressSpace.gmem)
                    ...
                    fc2_c_ptr = make_ptr(cutlass.BFloat16,
                                         ...
                                         cute.AddressSpace.gmem,
                                         assumed_align=16)
                    fc2_routing_scales_ptr = make_ptr(
                        cutlass.Float32, fc2_routing_scales.data_ptr(),
                        cute.AddressSpace.gmem)
                        ...
                        def _fc12_wrapper(
                            ...
                            a = cute.make_tensor(
                                ...
                                layout=cute.make_ordered_layout(
                                    (orig_m, k, 1), order=(1, 0, 2)))
                            b = cute.make_tensor(
                                b_ptr,
                                layout=cute.make_ordered_layout(
                                    (fc1_n, k, l), order=(1, 0, 2)))
                            ...
                            sfc = cute.make_tensor(
                                ...
                                layout=cute.make_ordered_layout(
                                    (32, 4, m // 128, 4, interm_size //
                                     (sf_vec_cx * 4), l),
                                    order=(2, 1, 4, 0, 3, 5)))
                            norm_const = cute.make_tensor(
                                norm_const_ptr, layout=cute.make_layout((1, )))
                            fc1_alpha = cute.make_tensor(
                                fc1_alpha_ptr, layout=cute.make_layout((l, )))
                            fc2_alpha = cute.make_tensor(
                                fc2_alpha_ptr, layout=cute.make_layout((l, )))
                            ...
                            ready = cute.make_tensor(ready_ptr,
                                                     layout=cute.make_layout(
                                                         (num_tiles, )))
                            fc1_sched = cute.make_tensor(
                                fc1_sched_ptr, layout=cute.make_layout((1, )))
                            fc2_sched = cute.make_tensor(
                                fc2_sched_ptr, layout=cute.make_layout((1, )))
                            w2 = cute.make_tensor(
                                w2_ptr,
                                layout=cute.make_ordered_layout(
                                    (fc2_n, fc2_k, l), order=(1, 0, 2)))
                            w2_sf = cute.make_tensor(
                                w2_sf_ptr,
                                layout=cute.make_ordered_layout(
                                    (32, 4, fc2_n // 128, 4, fc2_scale_k // 4,
                                     l),
                                    order=(2, 1, 4, 0, 3, 5)))
                            out = cute.make_tensor(
                                ...
                                layout=cute.make_layout(
                                    (num_tokens, fc2_n, 1),
                                    stride=(fc2_n, 1, num_tokens * fc2_n)))
            ...
            def _run_nvfp4_fc12_fused_rubin(
                ...
                inputs = [
                    ...
                    tile_idx_to_group_idx, tile_idx_to_mn_limit,
                    permuted_idx_to_expanded_idx, num_non_exiting_tiles,
                    global_sf, fc2_weight, fc2_weight_scale, fc2_alpha, output,
            ...
            def cute_dsl_nvfp4_fc12_fused_rubin(
                ...
                _run_nvfp4_fc12_fused_rubin(
                    ...
                    expanded_idx_to_permuted_idx, ep_size, enable_alltoall,
                    "trtllm::cute_dsl_nvfp4_fc12_fused_rubin",
                    ActivationType(activation_type), situ_beta,
                    situ_linear_beta)
